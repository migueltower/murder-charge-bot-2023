    ...
    while current <= end:
        ...
        while attempt < 3 and not success:
            try:
                ...
                success = True  # success on this attempt

            except requests.exceptions.RequestException as e:
                attempt += 1
                wait = min(60, 5 * (2 ** (attempt - 1)))
                print(f"{timestamp()} ⚠️ Network error on attempt {attempt} for {case_number}: {e}", flush=True)
                print(f"{timestamp()} ⏳ Waiting {wait} seconds before retrying...", flush=True)
                time.sleep(wait)
        ...
        print(f"{timestamp()} 💤 Sleeping for {sleep_duration:.2f} seconds to simulate human pacing...", flush=True)
